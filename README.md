...
- Uses incremental sync by default, including in mock mode, using the stored `etl_sync_state` watermark.
...
Run migrations and the default incremental sync:

```bash
python -m etl.main migrate
python -m etl.main sync
```

Use a full sync only when you need an initial backfill or a deliberate rebuild:
...
Re-running `sync --full` should be idempotent because upserts use natural keys such as `record_id`, `(record_id, month)`, `(record_id, screening_date)`, and `(record_id, session_number)`. Regular `sync` should only request and load records on or after the stored watermark.
...
Then validate counts and run logs:
...
SELECT COUNT(*) FROM redcap.distress_screenings;
SELECT COUNT(*) FROM redcap.wp6_sessions;
SELECT status, sync_type, records_extracted, records_loaded
